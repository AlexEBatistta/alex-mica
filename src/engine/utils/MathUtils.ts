import type { DisplayObject, IPointData, Rectangle } from "pixi.js";
...
export function setPivotToCenter(displayObject: DisplayObject, localBounds?: Rectangle, x?: boolean, y?: boolean): void {
	if (displayObject instanceof SDFBitmapText) {
		displayObject.anchor.set(0.5, 0.5);
	}
	x = x ?? true;
	y = y ?? true;
	localBounds = localBounds ?? displayObject.getLocalBounds();
	if (x) {
		displayObject.pivot.x = localBounds.width * 0.5 + localBounds.x;
	}
	if (y) {
		displayObject.pivot.y = localBounds.height * 0.5 + localBounds.y;
	}
}

export function magnitudeSquared(vector: Point): number {
	return Math.sqrt(Math.pow(vector.x, 2) + Math.pow(vector.y, 2));
}

export function multiplyScalar(vector: Point, num: number): Point {
