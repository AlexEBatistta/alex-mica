import type { LoadAsset, Loader } from "@pixi/assets";
...
export const loadCsv = {
	...
	async load(url: string, asset: LoadAsset): Promise<any> {
		let csvUrl = url;
		if (CSV_FROM_GOOGLE) {
	...
	testParse(_asset: string, options: LoadAsset): Promise<boolean> {
		return Promise.resolve(utils.path.extname(options.src).toLowerCase() === ".csv");
	},

	parse(asset: string, options: LoadAsset, _loader: Loader): Promise<any> {
		const parserOptions = { header: true, dynamicTyping: true, skipEmptyLines: true, ...(options?.data ?? {}), download: false, worker: false };
